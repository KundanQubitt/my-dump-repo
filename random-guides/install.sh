...
resolve_main_bundle() {
  ...
  # Download the HTML page directly — download URLs are embedded as href attributes
  # (the old approach of parsing a separate JS bundle no longer works).
  curl -fsSL --retry 3 -o "$html" "$DOWNLOAD_PAGE" \
    --header 'Accept-Encoding: gzip' \
    --compressed
  # If curl gave us a gzip blob (no --compressed support), decode it ourselves.
  if python3 -c "import sys; d=open(sys.argv[1],'rb').read(2); sys.exit(0 if d==b'\x1f\x8b' else 1)" "$html" 2>/dev/null; then
    local dec="$tmpdir/download.html.decoded"
    python3 -c "import gzip,sys,shutil; shutil.copyfileobj(gzip.open(sys.argv[1],'rb'),open(sys.argv[2],'wb'))" "$html" "$dec"
    mv "$dec" "$html"
  fi
  printf '%s\n' "$html"
}
...
resolve_download_from_bundle() {
  local html_file="$1"
  local product="$2"
  python3 - "$html_file" "$AG_PLATFORM" "$product" <<'PY'
import html as html_mod, re, sys
from pathlib import Path
from urllib.parse import unquote

raw = Path(sys.argv[1]).read_text(errors='replace')
# Unescape HTML entities (URLs may appear inside href attributes)
text = html_mod.unescape(raw)
# Normalize escaped forward-slashes (legacy JS artifact)
text = text.replace('\\/', '/')
...
if product == 'desktop':
    filename_patterns = [
        ...
        r'Antigravity%20Desktop\.tar\.gz',
    ]
    label = 'Antigravity 2.0'
elif product == 'ide':
    filename_patterns = [
        r'Antigravity%20IDE\.tar\.gz',
        r'Antigravity\+IDE\.tar\.gz',
        r'Antigravity%20IDE\.tar\.gz',
        r'AntigravityIDE\.tar\.gz',
    ]
...
for filename_re in filename_patterns:
    # Match URLs containing the platform slug followed by the expected filename.
    # Covers both href="..." in HTML and raw URLs in JS bundles.
    pattern = (r'https?://[^\s"\'>)\\]*/' +
               re.escape(platform) + r'/' + filename_re)
